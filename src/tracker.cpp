...
#include <stdexcept>
#include <regex>
bool TrackerClient::parse_url(const std::string& url, std::string& host, std::string& port,std::string& path) {
...
std::string TrackerClient::announce(const std::string& announce_url, const std::string& info_hash, long long total_length) {
    ...
    std::cout << "Initialing Tracker Connection" << std::endl;
    std::cout << "Target Host: " << host << " | Port: " << port << std::endl;
    ...
    try {
        ...
        std::cout << "Request transmitted. Awaiting response..." << std::endl;

        char read_buffer[4096];
        asio::error_code ec;

        while (true) {
            size_t bytes_transferred = socket.read_some(asio::buffer(read_buffer), ec);
        ...
        throw std::runtime_error("Network connection failed: " + std::string(e.what()));
    }
